...
w_mag = [sqrt(1), sqrt(2)];  %measurement noise pre_set up
...
% simulate the observation over time
    ...
    for t = 1 : Iteration
        ...
        X= F * X + [v_mag1 * randn*[(T^2/2); T; 0];v_mag2 * randn*[(T^2/2); T; 0]];             
...
%     save('JY_x_velocity_constant_ny_velocity','X_true','Z');
    
    load JY_x_velocity_constant_ny_velocity
...
title('Model 2: Constant Velocity');
a = gca;
a.FontSize = 15;
grid;
a.LineWidth= 1.5
...
xlabel('x'),ylabel('y');
legend('Ground Truth','Tracking Estimation');
title('Model 2: Constant Velocity Lane Changing');
a = gca;
a.FontSize = 15;
grid;
a.LineWidth= 1.5
...
error =[X_pred2(:,1),X_pred2(:,3)]-[X_true(:,1),X_true(:,4)];   
err0=sum(sum(abs(error)).^2)/Iteration
...
title('Model 4: Constatn accerleration')
xlabel('x'),ylabel('y');
legend('Ground Truth','Tracking Estimation');
a = gca;
a.FontSize = 15;
...
grid;
a.LineWidth= 1.5
error =[X_pred(:,1),X_pred(:,4)]-[X_true(:,1),X_true(:,4)];
err1 =sum(sum(abs(error)).^2)/Iteration
